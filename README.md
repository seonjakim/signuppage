...
터미널에 npm install을 치고 엔터를 누르면 CRA에 필요한 정보가 자동으로 install됩니다.( 저번에 그램을 사용하시던걸로 기억해서 window라고 예상합니다.)

터미널에 npm start라고 치시면 react가 실행됩니다.

보통 자동을 웹브라우저가 뜨지만 뜨지 않을 경우 => 사용하시는 웹브라우저에 들어가십니다.

웹브라우저 주소에 http://localhost:3000/
을 치시면 볼 수 있습니다.

종료하실때는 터미널에 Ctrl+c를 누르시면 종료됩니다.
